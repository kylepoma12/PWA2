...
// Checking the input data and add errors to $error array:
if(!$_POST['rusername'] || $_POST['rusername'] == 'username')
{
	$errors='Username or password is incorrect.';
}

if(!$_POST['rpassword'] || strlen($_POST['rpassword'])<6)
{
	$errors='Username or password is incorrect.';
}
...
if($_POST['fromLogin'])
{
	//does the error variable have any data?
	if($errors)
	{
...
//form is valid, verify password and return authenticated user ID:
	include_once('config.php');  //db connection data
	$mysqli = new mysqli($dbhost, $dbuser, $dbpass, $db);
	$stmt = $mysqli->prepare('SELECT `uid`, `password` from `users` where `username` = ? and `password` = ? LIMIT 1');
	$user = $_POST['rusername'];
    $pass = md5($_POST['rpassword']);
	$stmt->bind_param("ss", $user, $pass);
	$stmt->execute();
	$stmt->store_result();
	//check number of rows from query, if it's 1 then the username and password match
	$numrows = $stmt->num_rows;
	$stmt->bind_result( $uid, $passw );
	$quid = $stmt->fetch();
	$stmt->free_result();
	$stmt->close();
	
	if($numrows == 1){ //good login
			...
			foreach ($rows as $objects) {
				...
				echo "<p><span class='orange'>This project contains the following (<span class='blue'>".$numrows2."</span>) 
				tasks:</span> </p>";
				...
				foreach ($rows as $subt) {
					...
					echo "<li title='Click to Delete Task' class='subtask subicons'><a onClick='deleteTask(".$subt->tid.",".$objects->uid.");' 
					href='#'><span class='red pspan'>DELETE TASK&nbsp;<img class='deltask' src='images/cross-circle.png'/></span></a></li>";
				...
				echo '<p class="pdata"><div class="editicons"><a class="newt" href="#" title="Click to create a new task" 
				onClick="newTask('.$objects->pid.','.$objects->uid.');">NEW TASK&nbsp;<img id="newtask" src="images/plus-circle.png" title="Click for new project"/>
				</a><a onClick="deleteProject('.$objects->pid.', '.$objects->uid.');" href="#"><img id="delproj" src="images/cross-circle.png" 
				title="Click to Delete Project" /><span class="icontext">&nbsp;DELETE PROJECT</span></a></div></div></p></div>';
